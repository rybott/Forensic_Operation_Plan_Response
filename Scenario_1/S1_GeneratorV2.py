...
def Generate_Scenario_12(conn, 
    ...
    PO_df = pd.DataFrame(columns=['OrderID', 'Product', 'Vendor', 'Quantity', 'Shipment_Quarter'])
    ...
    PO_df['Quarter'] = PO_df['Shipment_Quarter'].dt.quarter
    PO_df['Day'] = PO_df['Quarter'].map(quarter_to_month)
    PO_df['Purchase_Date'] = pd.to_datetime(PO_df['Year'].astype(str) + '-' + PO_df['Day'] + '-01')
